        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400">
              Review flagged receipts and AI audit results.
            </p>
          </div>
          <button
          ...
          {audits.length > 0 ? (
                ...
                {audits.map((audit) => (
                  ...
                  >
                    <td className="px-6 py-4 font-medium text-white">
                      {audit.employee_id.slice(0, 8)}...
                    </td>
                    <td className="px-6 py-4 text-gray-300">
                      {audit.merchant || "Processing..."}
                    </td>
                    <td className="px-6 py-4 text-white font-medium">
                      ${Number(audit.amount || 0).toFixed(2)}
                    </td>
                    <td className="px-6 py-4">
                      <StatusBadge
                        status={audit.status || "pending"}
                        reason={audit.ai_decision_reason || undefined}
                      />
                    </td>
                    <td className="px-6 py-4 text-gray-400">
                      {audit.ai_confidence
                        ? `${(Number(audit.ai_confidence) * 100).toFixed(0)}%`
                        : "N/A"}
                    </td>
                    <td className="px-6 py-4">
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-gray-500">
                No pending receipts require review.
              </p>
            </div>
          )}
        </div>
            ...
            >
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="p-3 bg-[#111] rounded-lg border border-[#333]">
                      ...
                      <span className="text-sm font-medium text-white">
                        ${Number(selectedAudit.amount || 0).toFixed(2)}
                      </span>
                    </div>
                    <div className="p-3 bg-[#111] rounded-lg border border-[#333]">
                      <span className="text-xs text-gray-500 block">Date</span>
                      <span className="text-sm font-medium text-white">
                        {selectedAudit.receipt_date
                          ? new Date(
                              selectedAudit.receipt_date,
                            ).toLocaleDateString()
                          : new Date(
                    ...
                    <div className="p-3 bg-[#111] rounded-lg border border-[#333]">
                      ...
                      <span className="text-sm font-medium text-[#22D3EE]">
                        {selectedAudit.ai_confidence
                          ? `${(Number(selectedAudit.ai_confidence) * 100).toFixed(0)}%`
                          : "N/A"}
                      </span>
                    </div>
                  </div>
              ...
              <div className="p-6 border-t border-[#333] bg-[#111]">
                <p className="text-sm text-gray-400">
                  Approval actions are not available yet. This page currently
                  shows audits retrieved from the backend.
                </p>
              </div>
            </motion.div>
          </>
...
function StatusBadge({ status, reason }: { status: string; reason?: string }) {
  ...
  return (
    <span
      className={`px-2.5 py-0.5 rounded-full text-xs font-medium border capitalize ${styles[status] || styles.pending}`}
    >
      {status}
    </span>
  );
