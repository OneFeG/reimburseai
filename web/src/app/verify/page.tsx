...
export default function Verify2FAPage() {
  ...
  const complete2FA = (_data: unknown) => {};
  ...
  const parseErrorType = (message: string): ErrorType => {
    ...
    if (
      lowerMsg.includes("no email registered") ||
  ...
  const requestCode = async () => {
    ...
    try {
      const response = await api.post<TwoFARequestResponse>(
        "/auth/2fa/request",
        {
      ...
      >
        ...
        <div className="card p-8 border border-white/10 bg-navy-800/80 backdrop-blur-xl">
          ...
          {!codeSent &&
            !isSending &&
            error &&
            (error.type === "no_account" || error.type === "no_email") && (
              <motion.div
              ...
              >
                ...
                <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                  <p className="text-white/70 text-sm text-center">
                    {error.type === "no_account" ? (
                      <>
                        Your wallet is not registered in our system. Please
                        contact your company administrator to add you as an
                        employee, or{" "}
                        <Link
                          href="/#waitlist"
                      ...
                      <>
                        Your account exists but has no email address registered.
                        Please contact your company administrator to update your
                        profile with a valid email address.
                      </>
                    )}
                  </p>
                </div>
