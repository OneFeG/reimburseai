...
export function CompanySwitcher() {
  const { company, employee } = useProfile();
  const activeCompany = company ? { id: company.id, name: company.name } : null;
  const companies: CompanyMembership[] = activeCompany
    ? [
        {
          company_id: activeCompany.id,
          company_name: activeCompany.name,
          role: employee?.employee_role || "employee",
  ...
  const handleAddCompany = async () => {
    ...
    setAddError("Company switching is not enabled yet");
