...
async function main() {
    ...
    const contractKeysAddress = networkConfig[chainId].onchainID.keyManagerIdentity.reembolsoAI
...
/*
        const fnSig = "addClaim(uint256,uint256,address,bytes,bytes,string)"
        const selector = ethers.id(fnSig).slice(0, 10)

        const encodedArgs = ethers.AbiCoder.defaultAbiCoder().encode(
        ["uint256", "uint256", "address", "bytes", "bytes", "string"],
        [topic, scheme, issuer, signature, metadataHash, uri],
        )

        const data = selector + encodedArgs.slice(2)
    */
async function executeRequest({ issuer, to, value = 0, data } = {}) {
    const chainId = network.config.chainId
    const contractKeysAddress = networkConfig[chainId].onchainID.keyManagerIdentity.invoiceProvider

    // Conecta tu contrato en Fuji (sin redeploy, solo attach)
    const contractKey = await ethers.getContractAt("KeyManagerIdentity", contractKeysAddress, issuer)
    const contractClaims = await ethers.getContractAt("ClaimsIdentity", to)

    const valueWei = typeof value === "bigint" ? value : BigInt(value)
    const dataSignature = contractClaims.interface.encodeFunctionData("addClaim", [
        data.topic,
        data.scheme,
        data.issuerAddress,
        data.signature,
    ...
    const executionId = await contractKey.execute.staticCall(to, valueWei, dataSignature)
    const tx = await contractKey.execute(to, valueWei, dataSignature)
    const receipt = await tx.wait()
    console.log("✅ Transacción confirmada")
    console.log("🔍 Hash Tx:", receipt.hash)
    ...
    for (const log of receipt.logs || []) {
        if (!log.address || log.address.toLowerCase() !== contractKeysAddress.toLowerCase())
            continue
        try {
            const parsedLog = contractKey.interface.parseLog(log)
            parsed.push(parsedLog)
        } catch (_) {}
    }
    ...
    const execution = await contractKey.executionRequests(executionId)
...
async function approveExecute({ issuer, to, requestId, approve } = {}) {
    const { deployer, investor, entity } = await getNamedAccounts()
    const success = await contract.approve.staticCall(executionId, approve)
    const tx = await contract.approve(requestId, approve)
    const receipt = await tx.wait()
