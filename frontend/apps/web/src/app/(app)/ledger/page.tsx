...
import { ledgerApi, type LedgerEntry, type LedgerSummary, type LedgerEntryType, type LedgerEntryStatus } from "@/lib/api";
    ...
    const fetchLedgerData = async () => {
        ...
        try {
            const [entriesResponse, summaryResponse] = await Promise.all([
                ledgerApi.getByCompany(user.company.id, 100, 0, filter.type, filter.status),
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-400">Track all financial transactions and balances.</p>
                </div>
                <button
                        ...
                        <tbody className="divide-y divide-[#333]">
                            {entries.length > 0 ? (
                                entries.map((entry) => {
                                    ...
                                    const typeColor = typeColors[entry.entry_type] || "text-gray-500";
                                        ...
                                        <tr key={entry.id} className="hover:bg-[#1A1A1A] transition-colors">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    <div className={`p-1.5 rounded-lg bg-[#0A0A0A] border border-[#333] ${typeColor}`}>
                                            ...
                                            <td className="px-6 py-4 text-gray-400">
                                                {entry.fee_usd > 0 ? `$${entry.fee_usd.toFixed(2)}` : "-"}
                                            </td>
                                            <td className="px-6 py-4">
                                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border capitalize ${statusStyle}`}>
