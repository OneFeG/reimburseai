...
async def process_payout(body: PayoutRequest):
    ...
    if body.employee_id:
        try:
            employee = await get_employee(body.employee_id)
            if not employee:
                raise HTTPException(status_code=404, detail="Employee not found")

            # Verify wallet matches
            if (
                employee.get("wallet_address")
                and employee["wallet_address"].lower() != body.employee_wallet.lower()
            ):
                raise HTTPException(
                    status_code=400, detail="Wallet address does not match employee record"
                )
    ...
    if not is_whitelisted:
        raise HTTPException(status_code=403, detail="Wallet address is not whitelisted for payouts")
