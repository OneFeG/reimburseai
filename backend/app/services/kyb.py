...
class KYBService:
    ...
    async def submit_kyb(
        ...
        if existing.get("status") in ["pending", "approved"]:
            raise ValueError(f"KYB submission already exists with status: {existing['status']}")
    ...
    async def update_kyb_status(
        ...
        result = (
            client.table("kyb_submissions").update(update_data).eq("id", submission_id).execute()
        )
    ...
    async def update_kyb_data(
        ...
        **kwargs,
        ...
        if existing.get("status") not in ["unsubmitted", "rejected"]:
            raise ValueError(f"Cannot update KYB data with status: {existing['status']}")
        ...
        result = (
            client.table("kyb_submissions").update(update_data).eq("id", existing["id"]).execute()
        )
