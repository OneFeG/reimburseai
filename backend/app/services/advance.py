...
class AdvanceService:
    ...
    async def update_company_advance_config(
        ...
        update_data = {
            ...
            if credit_limit_usd is not None
            else existing.get("credit_limit_usd", self.DEFAULT_CREDIT_LIMIT_USD),
            "fee_bps": fee_bps
            if fee_bps is not None
            else existing.get("fee_bps", self.DEFAULT_FEE_BPS),
        ...
        # Upsert the config
        result = (
            client.table("advance_configs").upsert(update_data, on_conflict="company_id").execute()
        )
    ...
    async def request_advance(
        ...
        logger.info(f"Advance approved: ${amount_usd} for employee {employee_id} (fee: ${fee_usd})")
    ...
    async def get_advance(self, advance_id: str) -> dict[str, Any] | None:
        ...
        result = (
            client.table("advance_requests").select("*").eq("id", advance_id).single().execute()
        )
    ...
    async def update_advance_status(
        ...
        result = client.table("advance_requests").update(update_data).eq("id", advance_id).execute()
